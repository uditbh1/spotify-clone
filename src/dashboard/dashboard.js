...
const loadPlaylist=async(endpoint,elementId)=>{
    ...
    for(let{name,description,images,id} of items){
        ...
        playlistItem.innerHTML =`<img
                src="${imageUrl}" alt="${name}" class="rounded mb-2 object-contain shadow"
              />
...
const loadPlaylistTracks=({tracks})=>{
    ...
    for(let trackItem of tracks.items.filter(item=>item.track.preview_url)){
        ...
        playButton.className=`material-symbols-outlined play w-full absolute left-3 text-lg invisible`
...
document.addEventListener("DOMContentLoaded",async()=>{
    ...
    // const section={type:SECTIONTYPE.PLAYLIST,playlist:"37i9dQZF1DWXJfnUiYjUKT"}
